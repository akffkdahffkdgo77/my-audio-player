...
export default function Home() {
    ...
    // const handleVolume = (e: FormEvent<HTMLInputElement>) => {
    //     if (show && audio.current) {
    //         e.stopPropagation();
    //         audio.current.load();
    //         audio.current.volume = Number(e.currentTarget.value);
    //         setVolume(Number(e.currentTarget.value));
    //         setVolumeHeight(`calc(100% - ${Math.floor(parseFloat(e.currentTarget.value) * 100)}%)`);
    //     }
    // };
        ...
        <main className="w-full min-h-screen flex justify-center items-center flex-col gap-2.5">
            ...
            <div className="relative w-[300px] h-[500px] bg-black rounded-md p-5">
                <div className="absolute -right-[5px] flex flex-col gap-[0.5px]">
                    <button className="w-[5px] h-[30px] bg-black" aria-label="volume up" title="volume up" type="button" onClick={handleVolumeUp} />
                    <button className="w-[5px] h-[30px] bg-black" aria-label="volume down" title="volume down" type="button" onClick={handleVolumeDown} />
                </div>
                <div className="overflow-hidden w-full h-[150px] bg-blue-50 rounded-md border-2 border-black" role="button" tabIndex={-1} onKeyDown={onScreenClick} onClick={onScreenClick}>
